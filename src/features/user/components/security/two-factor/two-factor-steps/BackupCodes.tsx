...
function EmptyState({ className }: EmptyStateProps) {
 return (
  <Card className={cn("w-full max-w-sm shadow-xs", className)}>
   <CardHeader>
    <CardTitle className="flex items-center gap-2">Recovery codes</CardTitle>
    <CardDescription>
  ...
  <div className="flex flex-col items-center justify-center gap-4 rounded-lg border bg-muted/50 p-8 text-center">
   ...
   <div className="flex flex-col gap-2">
    ...
    <p className="text-muted-foreground text-xs">
     Generate recovery codes to use as a backup for two-factor authentication
    </p>
   </div>
   {onGenerate && (
    <Button
   ...
   <AlertDialogContent>
    <AlertDialogHeader>
     ...
     <AlertDialogDescription>
      ...
      Make sure to save the new codes in a secure location. This action cannot
      be undone.
     </AlertDialogDescription>
    </AlertDialogHeader>
    <AlertDialogFooter>
...
export default function AuthRecoveryCodes({
 ...
 return (
  <div>
   <Card className={cn("max-w-lg shadow-xs", className)}>
    <CardHeader>
      ...
      {codes.length > 0 ? (
       <>
        <Alert variant="destructive">
         ...
         <AlertDescription>
          Keep these codes somewhere safe. You will not be able to access these
          codes again. Each code works only once.
         </AlertDescription>
        </Alert>
       ...
       <EmptyStateWithGenerate isLoading={isLoading} onGenerate={onGenerate} />
      )}
     </div>
    </CardContent>
   </Card>
   <div className="flex items-center gap-5 mt-5">
    <p>If you have saved your recovery codes, you can leave this page.</p>
    <Link
