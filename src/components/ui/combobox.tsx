...
function ComboboxInput({
 ...
 return (
  <ComboboxPrimitive.Input
   ...
   className={cn(
    "flex h-9 w-full rounded-md bg-transparent placeholder:text-muted-foreground focus:outline-hidden disabled:cursor-not-allowed disabled:opacity-50 text-sm",
    className,
   )}
   {...props}
  />
 ...
 return (
  <ComboboxPrimitive.BadgeItem
   ...
   <ComboboxPrimitive.BadgeItemDelete
    ...
    className="shrink-0 rounded p-0.5 opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring data-highlighted:bg-destructive cursor-pointer hover:bg-primary-foreground duration-200">
    <X className="size-3" />
   </ComboboxPrimitive.BadgeItemDelete>
  </ComboboxPrimitive.BadgeItem>
 );
   ...
   <FocusScope asChild loop>
    <DismissableLayer>
     <ComboboxPrimitive.Content
      ...
      className={cn(
       "data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 relative z-[100] max-h-fit min-w-(--dice-anchor-width) origin-(--dice-transform-origin) rounded-md border bg-popover p-1 text-popover-foreground shadow-md data-[state=closed]:animate-out data-[state=open]:animate-in",
       className,
      )}
      {...props}>
      {children}
...
function ComboboxItem({
 ...
 return (
  <ComboboxPrimitive.Item
   ...
   className={cn(
    "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 text-sm outline-hidden data-disabled:pointer-events-none data-highlighted:bg-accent/70 data-highlighted:text-accent-foreground data-disabled:opacity-50 aria-selected:bg-accent",
   ...
   <ComboboxPrimitive.ItemIndicator
    className={cn(
     "absolute flex size-3.5 items-center justify-center text-primary",
