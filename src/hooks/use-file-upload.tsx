...
export const useFileUpload = (
 ...
 const addFiles = useCallback(
  (newFiles: FileList | File[]) => {
   ...
   if (!multiple) {
    // We can't call clearFiles() here because it sets state.
    // We'll handle clearing in the logic below.
    // But we should revoke URLs if needed.
    for (const file of state.files) {
     if (
      file.preview &&
      file.file instanceof File &&
 ...
 return [
  state,
  {
   ...
   openFileDialog,
   getInputProps,
   setFiles: useCallback((files: FileWithPreview[]) => {
    setState((prev) => ({
     ...prev,
     files,
     errors: [],
