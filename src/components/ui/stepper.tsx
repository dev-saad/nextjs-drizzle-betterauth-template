...
function StepperList(props: StepperListProps) {
 ...
 React.useEffect(() => {
  if (orientation === "horizontal" && listRef.current) {
   const activeItem = listRef.current.querySelector(
    `[data-state="active"][data-slot="stepper-item"]`,
   );
   if (activeItem) {
  ...
  <FocusContext.Provider value={focusContextValue}>
   <ListPrimitive
    ...
    className={cn(
     "flex outline-none",
     orientation === "horizontal"
      ? "w-full flex-row items-center gap-4 overflow-x-auto snap-x snap-mandatory scroll-smooth pb-4 lg:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] lg:overflow-visible lg:gap-6"
 ...
 return (
  <StepperItemContext.Provider value={itemContextValue}>
   <ItemPrimitive
    ...
    ref={ref}
    className={cn(
     "relative flex items-center lg:not-last:flex-1",
     orientation === "horizontal"
      ? "flex-row w-[70%] shrink-0 snap-center lg:w-auto"
      : "flex-col",
     className,
    )}>
    {children}
...
function StepperIndicator(props: StepperIndicatorProps) {
 ...
 // Don't wrap in motion if using asChild
 if (asChild) {
  const IndicatorPrimitive = Slot;
  return (
   <IndicatorPrimitive
    data-state={dataState}
 ...
 return (
  <m.div
   ...
   initial={false}
   animate={{
    scale: dataState === "active" ? [1, 1.1, 1] : 1,
   }}
   transition={{
    duration: 0.3,
    ease: "easeInOut",
 ...
 return (
  <SeparatorPrimitive
   ...
   <m.div
    ...
    animate={{
     scaleX: dataState === "completed" ? 1 : 0,
     scaleY: dataState === "completed" ? 1 : 0,
    }}
    transition={{
     duration: 0.5,
     ease: "easeInOut",
...
function StepperContent(props: StepperContentProps) {
 ...
 const targetIndex = stepKeys.indexOf(valueProp);
 const direction = targetIndex > currentIndex ? 1 : -1;

 // Use Slot if asChild, otherwise use animated div
 if (asChild) {
  if (valueProp !== value && !forceMount) return null;

  const ContentPrimitive = Slot;
  return (
   <ContentPrimitive
    id={contentId}
    role="tabpanel"
 ...
 // Always render AnimatePresence to allow exit animations
 return (
  <AnimatePresence mode="popLayout" initial={false}>
   {(valueProp === value || forceMount) && (
    <m.div
     key={valueProp}
     id={contentId}
     role="tabpanel"
