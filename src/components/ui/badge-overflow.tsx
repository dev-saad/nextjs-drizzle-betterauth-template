...
function BadgeOverflow<T = string>(props: BadgeOverflowProps<T>) {
 ...
 const { visibleItems, hiddenCount, hiddenItems } = React.useMemo(() => {
  if (!containerWidth || items.length === 0 || badgeWidths.size === 0) {
   return { visibleItems: items, hiddenCount: 0, hiddenItems: [] };
  }
  ...
  for (let i = 0; i < items.length; i++) {
   ...
   if (maxItems !== undefined && visible.length >= maxItems) {
    hidden.push(item);
    continue;
   }
       ...
       <TooltipProvider>
        <Tooltip>
         <TooltipTrigger asChild>
          <div className="cursor-pointer inline-flex h-5 shrink-0 items-center rounded-md border px-1.5 font-semibold text-xs hover:bg-muted">
           +{hiddenCount}
          </div>
         </TooltipTrigger>
         <TooltipContent>
