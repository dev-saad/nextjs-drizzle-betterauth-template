...
const SidebarHighlightStateContext = React.createContext<{
...
function Sidebar({
 ...
 if (collapsible === "none") {
  return (
   <SidebarHighlightStateContext.Provider value={{ setActiveId }}>
    <Highlight
     value={activeId}
   ...
   </SidebarHighlightStateContext.Provider>
  );
 }
...
>(
 (
  ...
  const highlightContext = React.useContext(SidebarHighlightStateContext);
  const id = React.useId();

  React.useEffect(() => {
   if (isActive) {
    highlightContext?.setActiveId(id);
   }
   return () => {
    if (isActive) {
